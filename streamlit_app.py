...
# 앱 제목 설정
st.set_page_config(page_title="TripMind - 여행 에이전트", page_icon="✈️", layout="wide")
...
with st.sidebar:
    ...
    API_URL = custom_api_url  # 사용자 지정 URL 사용
    ...
    5. **장소 검색**: "도쿄 근처의 좋은 식당 알려줘"
    6. **일정 공유**: "내 일정을 친구들에게 공유하고 싶어"
    """)
    ...
    # 추출된 여행 정보 표시 영역
    st.subheader("여행 정보")
    st.write("대화를 통해 추출된 정보가 여기에 표시됩니다.")
...
if prompt := st.chat_input("여행 계획에 대해 물어보세요..."):
    ...
    data = {
        "message": prompt,  # API 요구사항: message 필드
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not assistant_content:
                    assistant_content = "죄송합니다. 응답을 생성하는 중 오류가 발생했습니다."
                ...
                # 의도 정보 표시
                if "intent" in response_data and response_data["intent"]:
                    with st.sidebar:
                        st.write(f"**현재 의도:** {response_data['intent']}")
                
                # 다음 노드 정보 표시
                if "next_node" in response_data and response_data["next_node"]:
                    with st.sidebar:
                        st.write(f"**다음 노드:** {response_data['next_node']}")
            else:
...
with st.expander("디버그 정보", expanded=False):
    ...
    if st.button("API 테스트"):
        try:
            ...
            test_response = requests.post(API_URL, headers=test_headers, data=json.dumps(test_data))
