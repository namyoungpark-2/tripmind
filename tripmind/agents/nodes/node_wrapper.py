...
def node_wrapper(func: Callable[[Any, T], T]) -> Callable[[Any, T], T]:
    ...
    def wrapper(*args, **kwargs) -> T:
        ...
        try:
            # 진입 로그
            logger.info(f"{node_name} 시작: user_input={state.get('user_input', '')[:30]}...")
            # 세션 ID 추출
            ...
            if "response" not in result:
                ...
                if "messages" in result and isinstance(result["messages"], list):
                    ...
                    if assistant_messages:
                        ...
                        logger.info(f"{node_name}: assistant 메시지에서 response 필드 자동 설정")
            ...
            # 응답이 있는 경우 검증
            if "messages" in result and isinstance(result["messages"], list):
                assistant_message = next((msg for msg in result["messages"] if msg.get("role") == "assistant"), None)
                if assistant_message:
                    ...
                    if monitor_result and monitor_result.get("issues"):
                        ...
                        logger.warning(f"{node_name} 응답에서 {num_issues}개 이슈 감지됨")
                    ...
                    result["response"] = validate_node_response(node_name, assistant_message["content"])
                    ...
                    if "messages" in result and result["messages"]:
                        for i, msg in enumerate(result["messages"]):
                            if msg.get("role") == "assistant" and msg.get("content") == state.get("response"):
