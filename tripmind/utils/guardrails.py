...
class ResponseValidator:
    ...
    def validate_price_info(text: str) -> Tuple[str, List[str]]:
        ...
        for pattern in price_patterns:
            ...
            for match in matches:
                ...
                if "약" not in price_str and "대략" not in price_str and "보통" not in price_str:
                    # 정확한 가격 정보는 추측일 가능성이 높으므로 근사 표현으로 변경
                    replacement = f"약 {price_str}" 
                    modified_text = modified_text.replace(price_str, replacement)
                    issues.append(f"정확한 가격 정보를 근사 표현으로 변경: {price_str} → {replacement}")
        ...
        for pattern in operation_time_patterns:
            ...
            for match in matches:
                ...
                for word in certainty_words:
                    if word in time_str or re.search(r'{}[\s]?{}'.format(word, time_str), modified_text):
                ...
                if not has_certainty_word:
                    ...
                    issues.append(f"정확한 운영시간 표현을 완화: {time_str} → {replacement}")
    ...
    def remove_fabricated_contacts(text: str) -> Tuple[str, List[str]]:
        ...
        for email in emails:
            # 일부 공식 이메일은 허용
            if not any(domain in email for domain in ["tripmind.com", "example.com", "kakao.com", "google.com"]):
    ...
    def check_hallucinations(text: str, known_entities: List[str] = None) -> List[str]:
        ...
        for pattern in definitive_patterns:
            ...
            for match in matches:
                ...
                issues.append(f"확정적 주장 발견: '{context}' 부분에서 '{pattern}' 표현 사용")
    ...
    def validate_response(text: str) -> str:
        ...
        # 연락처 정보 제거
        modified_text, issues = ResponseValidator.remove_fabricated_contacts(modified_text)
        all_issues.extend(issues)
...
class NodeValidator:
            ...
            for msg in state["messages"]:
                if isinstance(msg, dict) and "role" in msg and "content" in msg:
                    ...
                    if msg["role"] == "assistant" and isinstance(msg["content"], str):
                        # AI 응답 내용 검증
                        msg["content"] = ResponseValidator.validate_response(msg["content"])
    ...
    def validate_response_for_node(node_name: str, response: str) -> str:
        ...
        if node_name == "sharing_node":
            ...
            # 캘린더 노드 특화 검증
            if "캘린더" in validated and "등록" in validated and "성공" not in validated:
